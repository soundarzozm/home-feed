...
async function run() {
  ...
  if (!handle || !password || !hostname || !publisherDid) {
    console.error('Error: Please populate BSKY_HANDLE, BSKY_PASSWORD, FEEDGEN_HOSTNAME, and FEEDGEN_PUBLISHER_DID in your .env file.');
    process.exit(1);
  }
  ...
  const response = await agent.api.com.atproto.repo.putRecord({
    ...
    record: {
      ...
      description: 'The essence of microblogging: no politics, no tech/programming content, and no promotional slop. Just everyday thoughts, feelings, jokes, and memes.',
